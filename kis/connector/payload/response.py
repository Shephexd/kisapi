...
class OverseaPresentBalanceResponse(APIResponse):
    class OverseaPresentBalanceResponse(Response):
        prdt_name: str = Field(alias="name", description="종목명")
        cblc_qty13: Decimal = Field(alias="holding_qty", description="잔고수량")
        thdt_buy_ccld_qty1: Decimal = Field(alias="exec_bid_qty", description="당일 매수 체결 완료 수량")
        thdt_sll_ccld_qty1: Decimal = Field(alias="exec_ask_qty", description="당일 매도 체결 완료 수량")
        ccld_qty_smtl1: Decimal = Field(alias="qty", description="체결기준 현재 잔고수량")
        ord_psbl_qty1: Decimal = Field(alias="available_qty", description="주문 가능한 주문 수량")
        frcr_pchs_amt: Decimal = Field(alias="buy_amt", description="해당 종목의 외화 기준 매입금액")
        frcr_evlu_amt2: Decimal = Field(alias="eval_amt", description="해당 종목의 외화 기준 평가금액")
        evlu_pfls_amt2: Decimal = Field(alias="eval_profit_loss", description="해당 종목의 매입금액과 평가금액의 외회기준 비교 손익")
        evlu_pfls_rt1: Decimal = Field(alias="eval_profit_loss_ratio", description="해당 종목의 평가손익을 기준으로 한 수익률")
        pdno: str = Field(alias="symbol", description="종목코드")
        bass_exrt: Decimal = Field(alias="exchange_rate", description="원화 평가 시 적용 환율")
        buy_crcy_cd: str = Field(alias="currency_code", description="매수통화코드")
        ovrs_now_pric1: Decimal = Field(alias="now_price", description="해당 종목의 현재가")
        avg_unpr3: Decimal = Field(alias="unit_bid_price", description="해당 종목의 매수 평균 단가")
        tr_mket_name: str = Field(alias="market_name", description="해당 종목의 거래시장명")
        natn_kor_name: str = Field(alias="nation", description="거래 국가명")
        pchs_rmnd_wcrc_amt: Decimal = Field(alias="eval_amt_krw", description="매입잔액원화금액")
        thdt_buy_ccld_frcr_amt: Decimal = Field(alias="exec_bid_amt", description="당일 매수 외화금액")
        thdt_sll_ccld_frcr_amt: Decimal = Field(alias="exec_ask_amt", description="당일 매도 외화금액")
        # unit_amt: Decimal = Field(description="단위금액")
        # std_pdno: str = Field(description="표준상품번호", repr=False)
        # prdt_type_cd: str = Field(description="상품유형코드", repr=False)
        scts_dvsn_name: str = Field(description="유가증권구분명")
        # loan_rmnd: Decimal = Field(description="대출 미상환 금액", repr=False)
        # loan_dt: str = Field(description="대출 실행일자", repr=False, )
        # loan_expd_dt: str = Field(description="대출 만기일자", repr=False)
        # ovrs_excg_cd: str = Field(alias="market_code", description="해외거래소코드")
        item_lnkg_excg_cd: str = Field(alias="market_code", description="종목연동거래소코드")

    class OverseaPresentCurrencyResponse(Response):
        crcy_cd: str = Field(alias="currency_code", description="통화코드")
        crcy_cd_name: str = Field(alias="currency_name", description="통화코드명")
        frcr_buy_amt_smtl: Decimal = Field(alias="bid_amt", description="외화매수금액합계(해당 통화로 매수한 종목 전체의 매수금액)")
        frcr_sll_amt_smtl: Decimal = Field(alias="ask_amt", description="외화매도금액합계(해당 통화로 매도한 종목 전체의 매수금액)")
        frcr_dncl_amt_2: Decimal = Field(alias="", description="외화예수금액2")
        frst_bltn_exrt: Decimal = Field(alias="exchange_rate", description="최초고시환율")
        frcr_buy_mgn_amt: Decimal = Field(alias="bid_margin_amt", description="외화매수증거금액(매수증거금으로 사용된 외화금액)")
        frcr_etc_mgna: Decimal = Field(alias="etc_margin_amt", description="외화기타증거금")
        frcr_drwg_psbl_amt_1: Decimal = Field(alias="withdrawable_usd", description="외화출금가능금액1(출금가능한 외화금액)")
        frcr_evlu_amt2: Decimal = Field(alias="withdrawable_krw", description="출금가능원화금액(출금가능한 원화금액)")
        # acpl_cstd_crcy_yn: str = Field(description="현지보관통화여부")
        nxdy_frcr_drwg_psbl_amt: Decimal = Field(alias="next_withdrawable_usd", description="익일외화출금가능금액")

    class OverseaPresentAccountResponse(Response):
        pchs_amt_smtl: Decimal = Field(alias="buy_asset_amt_usd", description="매입금액합계(해외유가증권 매수금액의 원화 환산 금액)")
        evlu_amt_smtl: Decimal = Field(alias="eval_asset_amt_usd", description="평가금액합계(해외유가증권 평가금액의 원화 환산 금액)")
        evlu_pfls_amt_smtl: Decimal = Field(alias="eval_return_usd", description="평가손익금액합계(해외유가증권 평가손익의 원화 환산 금액)")
        dncl_amt: Decimal = Field(alias="deposit_amt", description="예수금액")
        cma_evlu_amt: Decimal = Field(alias="cma_eval_amt", description="CMA평가금액")
        tot_dncl_amt: Decimal = Field(alias="liqudity_amt", description="총예수금액")
        etc_mgna: Decimal = Field(alias="etc_margin_amt", description="기타증거금")
        wdrw_psbl_tot_amt: Decimal = Field(alias="withdrawable_amt", description="인출가능총금액")
        frcr_evlu_tota: Decimal = Field(alias="eval_asset_amt_usd", description="외화평가총액")
        evlu_erng_rt1: Decimal = Field(alias="eval_asset_return_ratio", description="평가수익율1")
        pchs_amt_smtl_amt: Decimal = Field(alias="buy_asset_amt_krw", description="매입금액합계금액")
        evlu_amt_smtl_amt: Decimal = Field(alias="eval_asset_amt_krw", description="평가금액합계금액")
        tot_evlu_pfls_amt: Decimal = Field(alias="eval_return_krw", description="총평가손익금액")
        tot_asst_amt: Decimal = Field(alias="eval_account_amt", description="총자산금액")
        # buy_mgn_amt: Decimal = Field(description="매수증거금액")
        # mgna_tota: Decimal = Field(description="증거금총액")
        frcr_use_psbl_amt: Decimal = Field(alias="liqudity_amt_krw", description="외화사용가능금액")
        ustl_sll_amt_smtl: Decimal = Field(alias="executed_ask_amt", description="미결제매도금액합계")
        ustl_buy_amt_smtl: Decimal = Field(alias="executed_bid_amt", description="미결제매수금액합계")
        tot_frcr_cblc_smtl: Decimal = Field(alias="usd_liqudity_krw", description="총외화잔고합계")
        # tot_loan_amt: Decimal = Field(description="총대출금액")

    output1: List[OverseaPresentBalanceResponse] = Field(alias="assets")
    output2: List[OverseaPresentCurrencyResponse] = Field(alias="currency")
    output3: OverseaPresentAccountResponse = Field(alias="account")
